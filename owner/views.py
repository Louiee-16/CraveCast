...
def owner_dashboard_view(request):
    ...
    low_stock_items = Inventory.objects.filter(stock_qty__lte=20)
    ...
    for i in range(6, -1, -1):
        ...
        chart_labels.append(day.strftime('%b %d')) # Format: "Oct 01"
        
        daily_revenue = SalesRecord.objects.filter(sale_date=day).aggregate(
            total=Sum(F('quantity') * F('price'))
        )['total'] or 0
        chart_values.append(float(daily_revenue))
    ...
    context = {
        ...
        'low_stock_items': low_stock_items.order_by('stock_qty')[:5],
